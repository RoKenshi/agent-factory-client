...
В мастере укажите любой OpenAI-compatible endpoint, выберите модели-миньоны чекбоксами, назначьте
их ролям и выберите MCP-host. Codex и Claude регистрируются автоматически; для остальных клиентов
будет выдан готовый фрагмент конфигурации. Ключ провайдера хранится только в системном хранилище
учётных данных. Настройки, ограниченный кэш и очередь обезличенной синхронизации находятся в одном
приватном `agent-factory.json`; постоянной локальной SQLite-базы нет. MCP сам запускает локальный
runtime. Для возврата в интерфейс используйте `agent-factory open`.
